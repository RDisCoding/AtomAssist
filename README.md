...
Below is the high-level architecture of our microservices and cloud deployment:
...
### 4. Known Limitations
* **Cloud WebRTC Traversal**: While the entire application is fully deployed on AWS, strict enterprise AWS VPC firewall policies currently restrict the massive `UDP 40000-49999` port ranges required for WebRTC video traversal. The signaling connects perfectly, but video packets are dropped. **To experience the flawless video SFU, please run the application locally** where these firewall rules do not apply.
