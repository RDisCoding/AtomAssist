...
export default function SessionRoomPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  useEffect(() => {
    ...
    // Extract proper path for Nginx routing
    const mediaUrl = process.env.NEXT_PUBLIC_MEDIA_URL || 'http://localhost:5000';
    const mediaPath = mediaUrl.includes('/media') ? '/media/socket.io' : '/socket.io';
    
    const mSocket = io(mediaUrl, { 
